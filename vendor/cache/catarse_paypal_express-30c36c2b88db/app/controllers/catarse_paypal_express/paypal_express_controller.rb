class CatarsePaypalExpress::PaypalExpressController < ApplicationController
  ...
  def review
    contribution.update_attributes payment_method: 'PayPal'
  end
  ...
  def refund
    ...
    if refund_request.success?
      flash[:notice] = I18n.t('controllers.projects.contributions.refund.success')
    else
      flash[:alert] = I18n.t('controllers.projects.contributions.refund.error')
    end
  ...
  def ipn
    if contribution && notification.acknowledge
  ...
  def pay
    begin
      contribution.update_attributes payment_method: 'PayPal'
      response = gateway.setup_purchase(contribution.price_in_cents_with_tax, {
        ...
        currency_code: (PaymentEngines.configuration[:currency_charge] rescue nil) || 'BRL',
      ...
      contribution.update_attributes payment_token: response.token
  ...
  def success
    begin
      purchase = gateway.purchase(contribution.price_in_cents_with_tax, {
        ip: request.remote_ip,
